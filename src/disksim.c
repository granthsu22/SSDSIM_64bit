	...
	if (curr) {
		...
		if (curr->type == NULL_EVENT) {
			if ((disksim->iotrace) && io_using_external_event(curr)) {
				temp = io_get_next_external_event(disksim->iotracefile); // get from trace file
				if (!temp) {
...
static void prime_simulation ()
{
	...
	if (disksim->iotrace) {
		...
		if ((disksim->traceformat != VALIDATE) && (disksim->closedios == 0)) { // enter
			curr->type = NULL_EVENT;
...
{
	event *curr;
	// printf("disksim_simulate_event start\n");
	//	printf("overall_req_num: %d\n", overall_req_num);
	//	printf("disksim->totalreqs: %d\n", disksim->totalreqs);

	// check idle status
	if(in_idle_period) {
		if(pending_request > 0) {
			in_idle_period = 0;
			total_idle_time += simtime - idle_start_time;
			process_start_time = simtime;
			idle_start_time = -1;
		}
	} else {
		if(pending_request == 0) {
			in_idle_period = 1;
			idle_start_time = simtime;
			total_process_time += simtime - process_start_time;
	...
	if ((curr = getnextevent()) == NULL) {
		disksim_simstop ();
		if(idle_start_time != -1) {
			total_idle_time += simtime - idle_start_time;
		}
		if(process_start_time != -1) {
			total_process_time += simtime - process_start_time;
		}
		...
		ASSERT(curr != NULL);
		simtime = curr->time; // event-driven
//		printf("disksim_simulate_event: %lf\n", simtime);
	...
	//	printf("disksim_simulate_event end\n");
...
int disksim_initialize_disksim_structure (struct disksim *disksim)
{
	...
	// wendy: initialization
	first = 1;
	timeslice_counter = 1;
	pending_request = 0;
	first_enter_iotrace = 1;

	acc_light_loading_time = 0.0;

	//for time calculating
	total_complete_req = 0; 
	all_response_time = 0.0;
	ssd_response_time = 0.0;
	overall_response_time = 0.0;
	overall_total_complete_req = 0;
	write_queueing_time = 0.0;
	read_queueing_time = 0.0;

	host_big_read_num = 0;
	host_big_write_num = 0;

	find_light_times = 0;

	all_worst_response_time = 0;
	ssd_worst_response_time = 0;

	NCQ_command_size = 0;
	ssd_write_buffer_size = 0;
	ssd_read_buffer_size = 0;

	for(i = 0; i < PROCESS_NUM; i++) {
		read_overall_response_time_per_process[i] = 0;
		write_overall_response_time_per_process[i] = 0;
		overall_service_time_per_process[i] = 0;
		read_req_num_per_process[i] = 0;
		write_req_num_per_process[i] = 0;
		req_num_per_process[i] = 0;
	}

	for(i = 0; i < MAX_DIE_NUM; i++) {
		waiting_pool_die_condition[i] = 0;
	}

	addtoextraq_counter = 0;
	getfromextraq_counter = 0;
	anticipate_event = 0;

	fios_process_time_total = 0;
	fios_process_time_start = 0;
	fios_process_flag = 0;
...
{
	if(type & READ) {
		read_req_num_per_process[pid]++;
		read_overall_response_time_per_process[pid] += (end_time - begin_time);
	} else {
		write_req_num_per_process[pid]++;
        /*if(pid == 2){
            printf("begin: %lf, end: %lf\n", begin_time,end_time);
            printf("simtime: %lf, write_res: %lf\n",simtime,end_time - begin_time);
			printf("in_read: %d, in_anti: %d\n" , in_read_anticipation_period, in_read_anticipation_period);
        }*/
		write_overall_response_time_per_process[pid] += (end_time - begin_time);
	}
}

void all_service_time_calculate_per_process(int pid, double begin_time, double end_time)
{
	req_num_per_process[pid]++;
	overall_service_time_per_process[pid] += (end_time - begin_time);
}
...
{
	double tmp;
	tmp = end_time - begin_time;
	if(tmp > all_worst_response_time) {
		all_worst_response_time = tmp;
	}

	all_response_time += tmp;
}
...
void ssd_time_calculate(double begin_time, double end_time, u_int flags)
{
	double tmp;
	tmp = end_time - begin_time;
	if(tmp > ssd_worst_response_time) {
		ssd_worst_response_time = tmp;
	}

	ssd_response_time += tmp;

	if(flags & READ) {
		read_ssd_time += end_time - begin_time;
	} else {
		write_ssd_time += end_time - begin_time;
	}
...
{
	overall_response_time += end_time - begin_time;
}

void queueing_time_calculate(double begin_time, double end_time, u_int flags)
{
	if(flags & READ) {
		read_queueing_time += end_time - begin_time;
	} else {
		write_queueing_time += end_time - begin_time;
	}
}

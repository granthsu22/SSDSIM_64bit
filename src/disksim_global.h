...
// add by wendy
#define FAKE_EVENT -100
#define ANTICIPATE_EVENT -200
#define OVERALL_QUEUE 100
#define IODRIVER_QUEUE 200
#define SSD_QUEUE 300
...
#define DISKSIM_EVENT_SIZE	320 // ori: 160
...
/* added by gengyouchen */
struct ssd_config {
	int number_of_sectors_per_page;
	int number_of_pages_per_block;
	int number_of_blocks_per_plane;
	int number_of_planes_per_die;
	int number_of_dice_per_channel;
...
int gc_write_num;
int erase_num;
int timeslice_counter;
int first;

int host_big_read_num;
int host_big_write_num;

double acc_light_loading_time;
...
typedef struct ioreq_ev {
   ...
   int ppn;
   double arrival_time;
   enum ssd_req_type access_type;
   enum ssd_req_phase access_phase;
   enum ssd_req_initiator access_initiator;
   enum ssd_req_period access_period; // wendy
   struct ioreq_ev *next_waiting;
   struct ioreq_ev *prev_waiting;

   // add by wendy
   int reasonable; // judge the req is reasonable or not: 1->reasonable; 0->unreasonable
   int pid; // application id for each request
   double trace_time;
   double starttime_iobuf; // for time calculating
   double time_iob_un; // for time calculating
   double dispatch_time; // for FIOS to handle requests whose timeslice is more than limited
   int exceed_timeslice; // for FIOS to handle requests whose timeslice is more than limited

   double ssd_arrive_time;
   double ssd_queue_enter_time;

   int exceed_deadline;
   int read_over_write_times;
   int exceed_starve;
} ioreq_event;
...
/* opaque structures for different modules */
  ...
  // wendy: use for parameter file
  int fairness_scheduler; // NOOP(0), CFQ(1), FIOS(2), ORACLE(3), WENDY(4)
  int use_pid_trace; // use: 1, not use: 0
  int chosen_pid; // -1: all processes run together
  int chosen_type; // -1: read/write run together
  int process_num; // total process number
  double timeslice; // total timeslice that each process has
  double read_timeslice; // reduce timeslice when read request dispatches
  double write_timeslice; // reduce timeslice when write request dispatches
  int curr_over_next;
  int read_over_write;
  int host_over_gc;
  int ncq_command_size_bound;
  int dispatch_threshold; // judge light loading dies
  int internal_scheduler; // internal scheduler in ssd
  double write_deadline; // write_deadline for wendy scheduler
  int io_write_deadline_scheduler;
  int internal_write_deadline_scheduler;
  int write_starve;
  int io_write_starve;
  int internal_write_starve;
} disksim_t;
...
// add by wendy
#define DISPATCH_THERSHOLD (disksim->dispatch_threshold)
#define PROCESS_NUM 20

#define TIMESLICE (disksim->timeslice)
#define MAX_DIE_NUM 256

#define ANTICIPATION_TIME 0.675

#define NCQ_COMMAND_SIZE_BOUND (disksim->ncq_command_size_bound)
#define BUFFER_SIZE_BOUND 524288 // 256MB = 262144KB = 524288sectors

#define USE_PID_TRACE (disksim->use_pid_trace)
#define CHOSEN_PID (disksim->chosen_pid)
#define CHOSEN_TYPE (disksim->chosen_type)

#define QUOTA 10


#define READ_TIMESLICE (disksim->read_timeslice)
#define WRITE_TIMESLICE (disksim->write_timeslice)
#define DELAY_DISPATCH 0

#define CURR_OVER_NEXT (disksim->curr_over_next)
#define READ_OVER_WRITE (disksim->read_over_write)
#define HOST_OVER_GC (disksim->host_over_gc)

#define ACTIVE_THRESHOLD (disksim->active_threshold)
#define ANTICIPATE_THRESHOLD (disksim->anticipate_threshold)
#define ANTICIPATE_SHORT_THRESHOLD (disksim->anticipate_short_threshold)

#define FAIRNESS_SCHEDULER (disksim->fairness_scheduler)
#define NOOP 0
#define CFQ 1
#define FIOS 2
#define ORACLE 3
...
#define INTERNAL_SCHEDULER (disksim->internal_scheduler)
#define WRITE_DEADLINE (disksim->write_deadline)
#define IO_WRITE_DEADLINE_SCHEDULER (disksim->io_write_deadline_scheduler)
#define INTERNAL_WRITE_DEADLINE_SCHEDULER (disksim->internal_write_deadline_scheduler)
#define WRITE_STARVE (disksim->write_starve)
#define IO_WRITE_STARVE (disksim->io_write_starve)
#define INTERNAL_WRITE_STARVE (disksim->internal_write_starve)

int find_light_times;
...
int total_complete_req;
int overall_total_complete_req;
double all_response_time;
double ssd_response_time;
double overall_response_time;
double write_queueing_time;
double read_queueing_time;

double all_worst_response_time;
double ssd_worst_response_time;

double read_ssd_time;
double write_ssd_time;

void all_time_calculate(double begin_time, double end_time);
void ssd_time_calculate(double begin_time, double end_time, u_int flags);
void overall_time_calculate(double begin_time, double end_time);
void queueing_time_calculate(double begin_time, double end_time, u_int flags);

void all_time_calculate_per_process(int pid, int type, double begin_time, double end_time);
void all_service_time_calculate_per_process(int pid, double begin_time, double end_time);

int NCQ_command_size;
int ssd_write_buffer_size;
int ssd_read_buffer_size;

double die_time_accumulate[MAX_DIE_NUM];
int waiting_pool_die_condition[MAX_DIE_NUM];

double read_overall_response_time_per_process[PROCESS_NUM];
double write_overall_response_time_per_process[PROCESS_NUM];
double overall_service_time_per_process[PROCESS_NUM];
int read_req_num_per_process[PROCESS_NUM];
int write_req_num_per_process[PROCESS_NUM];
int req_num_per_process[PROCESS_NUM];

int addtoextraq_counter;
int getfromextraq_counter;

int anticipate_event;

double fios_process_time_total;
double fios_process_time_start;
int fios_process_flag;

int anti_counter;
int read_anti_counter;
int anti_good;
int read_anti_good;
int anti_dispatch;
int read_anti_dispatch;

double replenish_time;

/*ADD BY GRANT*/
double anti_time;
double anti_read_time;

double idle_start_time; 
double total_idle_time;
double process_start_time;
double total_process_time;

int ioqueue_request_num;
int in_idle_period;
int in_anticipation_period;
int in_read_anticipation_period;

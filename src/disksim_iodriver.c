...
{
	//	printf("iodriver_send_event_down_path: blkno: %d, simtime: %lf, trace_time: %lf\n", curr->blkno, simtime, curr->trace_time);
	intchar busno;
	intchar slotno;
...
static int check_send_out_request (iodriver *curriodriver, int devno)
{
	...
	//	printf("check_send_out_request devno: %d\n", devno);
	...
	if (curriodriver->usequeue == TRUE) {
		...
		if (queuectlr != -1) {
			...
			//			printf("max: %d\n", curriodriver->devices[devno].maxoutstanding);
			//return(numout < curriodriver->devices[devno].maxoutstanding);
			return(numout < 1000000); // wendy
		}
	} else {
		return(!numout);
	}
...
static ioreq_event * handle_new_request (iodriver *curriodriver, ioreq_event *curr)
{
	...
	//	printf("handle_new_request::  time %f, devno %d, blkno %d, bcount %d\n",simtime, curr->devno, curr->blkno, curr->bcount);
...
{
	//	printf("iodriver_schedule: blkno: %d, simtime: %lf, time: %lf\n", curr->blkno, simtime, curr->trace_time);
	ctlr *ctl;
	/*
...
void iodriver_access_complete (int iodriverno, intr_event *intrp)
{
	...
	//	printf("iodriver_access_complete pid: %d blkno: %d\n", req->pid, req->blkno);

	disksim_exectrace("Request completion: simtime %f, devno %d, blkno %d, time %f\n", 
			simtime, req->devno, req->blkno);

	if (iodrivers[iodriverno]->devices[(req->devno)].queuectlr != -1) // never enter
	{
	...
	else if (disksim->closedios) { // never enter
...
{
	//	printf("iodriver_respond_to_device\n");
	...
	req = (ioreq_event *) intrp->infoptr;
	//	printf("blkno: %d, simtime: %lf, time: %lf\n", req->blkno, simtime, req->trace_time);
	/*
...
{
	//	printf("iodriver_interrupt_complete\n");
	/*
...
{
	//	printf("iodriver_interrupt_arrive\n");
	event *tmp;
...
{
	//printf("iodriver_request: blkno: %d, simtime: %lf, time: %lf, pid: %d\n", curr->blkno, simtime, curr->trace_time, curr->pid);
	...
	ret = ioreq_copy (curr);
	//	printf("ioqueue_add_request to overallqueue: pid: %d blkno: %d trace_time: %d\n", ret->pid, ret->blkno, ret->trace_time);
	ASSERT(ret != NULL);
...
{
	//	printf("get_device_maxoutstanding\n");
...
void iodriver_initialize (int standalone)
{
	...
	overallqueue->debug_name = OVERALL_QUEUE;
	...
	for (i = 0; i < numiodrivers; i++) {
		...
		for (j = 0; j < numdevs; j++) {
			...
			currdev->queue->debug_name = IODRIVER_QUEUE; // add by wendy

...
#include "ssdmodel/ssd.h" // wendy
#include "../ssdmodel/ssd_fsm.h"
#include "../ssdmodel/ssd_ftl.h"
...
int ioqueue_get_number_pending (ioqueue *queue)
{
	...
	//printf("ioqueue_get_number_pending queue->debug_name %d numpend %d\n", queue->debug_name, numpend);
	return(numpend);
}
...
{
	int numout = 0;
	//	printf("ioqueue_get_reqoutstanding  queue->debug_name: %d\n", queue->debug_name);
	//	printf("reqoutstanding    base: %d, timeout: %d, priority: %d\n", queue->base.reqoutstanding, queue->timeout.reqoutstanding, queue->priority.reqoutstanding);
	//	printf("numoutstanding    base: %d, timeout: %d, priority: %d\n", queue->base.numoutstanding, queue->timeout.numoutstanding, queue->priority.numoutstanding);
...
static void ioqueue_get_cylinder_mapping(ioqueue *queue, iobuf *curr, int blkno, int *cylptr, int *surfptr, int cylmaptype)
{
	cylmaptype = (cylmaptype == -1) ? queue->cylmaptype : cylmaptype;
//	printf("queue: %d, cylmaptype: %d\n", queue->debug_name, cylmaptype);
	switch (cylmaptype) {
		case MAP_NONE:
			*cylptr = 0;
			*surfptr = 0;
...
static void ioqueue_update_subqueue_statistics (subqueue *queue)
{
	...
	//	printf("debug_name: %d\n", queue->bigqueue->debug_name);
	//	printf("ioqueue_update_subqueue_statistics  numoutstanding: %d maxoutstanding: %d\n", queue->numoutstanding, queue->maxoutstanding);
	if (queue->numoutstanding > queue->maxoutstanding) {
		queue->maxoutstanding = queue->numoutstanding;
		//	printf("change!! ioqueue_update_subqueue_statistics  numoutstanding: %d maxoutstanding: %d\n", queue->numoutstanding, queue->maxoutstanding);
	}
...
{
	//	printf("time: %f, blkno: %d, bcount: %d\n", temp->starttime, temp->blkno, temp->totalsize);
	int concatmax;
...
static double service_time(unsigned int current_cyl,unsigned int current_sector, unsigned int current_head,unsigned int target_cyl,unsigned int target_sector, unsigned int target_head, int read, unsigned int blocks, disk *currdisk)
{
	int i;
	double seektime = 0.0;
	double rotation = 0.0;
...
{
	//		printf("ioreq_event * ioqueue_get_next_request_from_subqueue (subqueue *queue): sche_alg=%d\n", queue->sched_alg);
	iobuf *temp = NULL;
#if 0
	iobuf *temp_sdf = NULL;
	iobuf *temp_sptf = NULL;
	...
	//	printf("ioqueue_get_next_request_from_subqueue: queue->debug_name: %d reqoutstanding: %d blkno: %d\n", queue->bigqueue->debug_name, queue->reqoutstanding, temp->blkno);
		...
		case BATCH_FCFS:
			if (temp->batchno != -1) {
				...
				while (batch_ptr->batchno == temp->batchno) {
					...
					queue->reqoutstanding++;
					//					printf("ioqueue_get_specific_request_from_subqueue: queue->reqoutstanding: %d blkno: %d\n", queue->reqoutstanding, temp->blkno);
...
static ioreq_event * ioqueue_remove_completed_request (subqueue *queue, ioreq_event *done)
{
	...
	// wendy
	if(queue->bigqueue->debug_name == OVERALL_QUEUE) {
		if(done->pid != FAKE_EVENT) {
			overall_total_complete_req++;
			overall_time_calculate(done->trace_time, simtime);
		}
	} else if(queue->bigqueue->debug_name == SSD_QUEUE) {
		if(done->pid != FAKE_EVENT) {
			total_complete_req++;
			ssd_time_calculate(done->ssd_queue_enter_time, simtime, done->flags);
			all_time_calculate(done->trace_time, simtime);
			all_time_calculate_per_process(done->pid, done->flags, done->trace_time, simtime);
		}
	}
	...
	ioqueue_update_subqueue_statistics(queue);
	tmp = queue->current;
	// ASSERT (tmp != NULL); //yao-qing: ooo
	/* NOTE- the following match will fail with concat unless fixed properly */
	if ((tmp == NULL) || (tmp->state != PENDING) || (ioqueue_request_match(done, tmp) == 0)) { // yao-qing: ooo
		tmp = queue->list->next;
		tail = queue->list;
	...
	if (tmp->reqcnt == 1) {
		...
		queue->iobufcnt--;
		queue->reqoutstanding--;
		//		printf("ioqueue_remove_completed_request: queue->bigqueue->debug_name: %d queue->reqoutstanding: %d blkno: %d pid: %d\n", queue->bigqueue->debug_name, queue->reqoutstanding, done->blkno, done->pid);
		queue->listlen--;
		if (tmp->flags & READ) {
			queue->readlen--;
		}
		...
		if(tmp == queue->current) //yao-qing: ooo
			queue->current = NULL;
	} else { // never enter
		if (queue->sched_alg == BATCH_FCFS) {
			...
			//			printf("NEVER ioqueue_remove_completed_request: queue->reqoutstanding: %d blkno: %d\n", queue->reqoutstanding, done->blkno);
			queue->listlen--;
			if (trv->flags & READ) {
				queue->readlen--;
			}
			...
			queue->iobufcnt--;
			queue->reqoutstanding--;
			//		printf("ioqueue_remove_completed_request: queue->reqoutstanding: %d blkno: %d\n", queue->reqoutstanding, done->blkno);
		...
		ioreq_event *wanted)
{
	...
	//	printf("ioqueue_get_specific_request  queue->debug_name: %d\n", queue->debug_name);
	/*
...
ioreq_event * ioqueue_get_next_request (ioqueue *queue)
{
	 ...
	 */
	//	printf("ioqueue_get_next_request: debug_name %d\n", queue->debug_name);
	if ((queue->priority.listlen - queue->priority.numoutstanding) > 0) {
	...
	if (!tmp && ((queue->base.listlen - queue->base.numoutstanding) > 0)) { // enter
		if ((tmp = ioqueue_get_next_request_from_subqueue(&queue->base))) {
			queue->lastsubqueue = IOQUEUE_BASE;
		}
	}
	if (tmp) {
		//printf("idle die number:%d at %lf\n",die_state_print(), simtime);
		if(queue->debug_name == SSD_QUEUE) {
			totalreq_in_ioqueue--;
			queueing_time_calculate(tmp->trace_time, simtime, tmp->flags);
		}

		//printf("time: %f, blkno: %d, bcount: %d\n", tmp->trace_time, tmp->blkno, tmp->bcount);
		int numout = queue->base.numoutstanding + queue->timeout.numoutstanding + queue->priority.numoutstanding;
		//		printf("ioqueue_get_next_request  numoutstanding: %d maxoutstanding: %d\n", numout, queue->maxoutstanding);
		if (numout > queue->maxoutstanding) {
			queue->maxoutstanding = numout;
			//		printf("change!! ioqueue_get_next_request  numoutstanding: %d maxoutstanding: %d\n", numout, queue->maxoutstanding);
		}
		tmp->time = 0.0;
	}
	//	printf("ioqueue_get_next_request end\n");
	/*
	   fprintf (outputfile, "Exiting ioqueue_get_next_request: %d\n", ((tmp) ? tmp->blkno : -1));
	 */
	//printf("idle die number:%d\n at %lf\n",die_state_print(), simtime);
	return(tmp);
}
...
double ioqueue_add_new_request (ioqueue *queue, ioreq_event *new)
{
	...
	if(queue->debug_name == SSD_QUEUE) {
		totalreq_in_ioqueue++;
		new->ssd_queue_enter_time = simtime;
	}

	if(queue->debug_name == IODRIVER_QUEUE) {
		//		printf("ioqueue_add_new_request  blkno: %d pid: %d trace_time: %lf\n", new->blkno, new->pid, new->trace_time);
	}
//	printf("ioqueue_add_new_request  queue->debug_name: %d blkno: %d pid: %d\n", queue->debug_name, new->blkno, new->pid);
	iobuf *tmp;
	int listlen;
	int qlen;
	int readlen;
	//	printf("ioqueue_add_new_request:: time %lf devno %d blkno %d bcount %d\n", simtime, queue->devno, new->blkno, new->bcount);
	/*
	 ...
	 */
//	printf("ioqueue_add_new_request end\n");
	return(0.0);
}
...
{
	//	printf("ioqueue_physical_access_done  queue->debug_name: %d\n", queue->debug_name);
	ioreq_event *ret;
	/*
	...
	ASSERT(ret != NULL);
	if (queue->comboverlaps) { // never enter
...
/*
 * The followings are added by wendy
 * CFQ, FIOS as baseline implementation
 */

// CFQ implementation
void cfq_replenish(void)
{
	int i;
	for(i = 0; i < PROCESS_NUM; i++) {
		cfq_process_queue[i].timeslice = TIMESLICE;
		cfq_process_queue[i].quota = QUOTA;
	}
}

void cfq_queue_initialize(void)
{
	int i;
	...
	for(i = 0; i < PROCESS_NUM; i++) {
		cfq_process_queue[i].pid = i;
		cfq_process_queue[i].timeslice = TIMESLICE;
		cfq_process_queue[i].starttime = 0;
		cfq_process_queue[i].quota = QUOTA;
		cfq_process_queue[i].req_num = 0;
		cfq_process_queue[i].head = cfq_process_queue[i].tail = NULL;
		cfq_process_queue[i].active = 0;
	}
}

void cfq_queue_print(void)
{
	int i;
	for(i = 0; i < PROCESS_NUM; i++) {
		printf("%d ", cfq_process_queue[i].req_num);
	}
	printf("\n");
}

void cfq_queue_add_new_request(struct ioq *queue, ioreq_event *curr)
{
	totalreq_in_ioqueue++;
	//	printf("cfq_queue_add_new_request: queue->debug_name %d  ", queue->debug_name);
	//	printf("cfq_queue_add_new_request  pid: %d, simtime: %lf, trace_time: %lf, blkno: %d\n", curr->pid, simtime, curr->trace_time, curr->blkno);
	//// for original time calculating ////
	int listlen;
	int qlen;
	int readlen;
	curr->time = simtime;
	curr->starttime_iobuf = -1.0;
	ioqueue_update_arrival_stats(queue, curr);

	// update original subqueue info
	ioqueue_update_subqueue_statistics(&queue->base);
	queue->base.iobufcnt++;
	queue->base.listlen++; // COULD BE A BUG!
	if (curr->flags & READ) {
		queue->base.readlen++;
		queue->base.numreads++;
	} else {
		queue->base.numwrites++;
	}

	// update original queue info
	listlen = queue->base.listlen + queue->timeout.listlen + queue->priority.listlen;
	qlen = listlen - (queue->base.numoutstanding + queue->timeout.numoutstanding + queue->priority.numoutstanding);
	readlen = queue->base.readlen + queue->timeout.readlen + queue->priority.readlen;
	queue->maxlistlen = max(listlen, queue->maxlistlen);
	...
	//// end original time calculating ////

	int pid = curr->pid;
	ASSERT(pid >= 0);
	cfq_process_queue[pid].pid = pid;

	/*
	// sort by time
	if(cfq_process_queue[pid].head == NULL && cfq_process_queue[pid].tail == NULL) {
	curr->prev = curr->next = NULL;
	cfq_process_queue[pid].head = curr;
	} else {
	curr->prev = curr->next = NULL;
	cfq_process_queue[pid].tail->next = curr;
	curr->prev = cfq_process_queue[pid].tail;
	}

	cfq_process_queue[pid].tail = curr;
	 */
	pending_request++;
	ioqueue_request_num++;
	...
	if(cfq_process_queue[pid].head == NULL && cfq_process_queue[pid].tail == NULL) {
		...
		ASSERT(cfq_process_queue[pid].head == NULL);
		ASSERT(cfq_process_queue[pid].tail == NULL);

		curr->prev = curr->next = NULL;
		cfq_process_queue[pid].head = curr;
		cfq_process_queue[pid].tail = curr;

	} else if(curr->blkno < cfq_process_queue[pid].head->blkno) {

		ASSERT(cfq_process_queue[pid].head != NULL);
		ASSERT(cfq_process_queue[pid].tail != NULL);

		cfq_process_queue[pid].head->prev = curr;
		curr->prev = NULL;
		curr->next = cfq_process_queue[pid].head;
		cfq_process_queue[pid].head = curr;

	} else if(curr->blkno >= cfq_process_queue[pid].tail->blkno) {
		...
		ioreq_event *tmp;
		tmp = cfq_process_queue[pid].head;
		while(tmp->blkno <= curr->blkno) {
			tmp = tmp->next;
		}
	...
	cfq_process_queue[pid].req_num++;
	//	cfq_queue_print();
}

ioreq_event* cfq_queue_remove_from_head(int pid)
{
	ioreq_event *tmp = NULL;
	if(pid == -1) {
		return NULL;
	...
	//	printf("cfq_queue_remove_from_head  ");
	ASSERT(pid != -1);
	tmp = cfq_process_queue[pid].head;

	ASSERT(tmp != NULL);
	cfq_process_queue[pid].head = cfq_process_queue[pid].head->next;

	if(cfq_process_queue[pid].head == NULL) {
		cfq_process_queue[pid].head = cfq_process_queue[pid].tail = NULL;
	} else {
		cfq_process_queue[pid].head->prev = NULL;
	}

	cfq_process_queue[pid].req_num--;
	cfq_process_queue[pid].quota--;
	//	printf("pid: %d, simtime: %lf, trace_time: %lf, blkno: %d\n", tmp->pid, simtime, tmp->trace_time, tmp->blkno);

	//	cfq_queue_print();

	if(tmp != NULL) {
		tmp->ssd_arrive_time = simtime;
	}
...
ioreq_event* cfq_queue_get_next_request(struct ioq *queue)
{
	ioreq_event *tmp = NULL;
	...
	//	printf("cfq_queue_get_next_request: %lf\n", simtime);
	// FIFO
	/*for(i = 0; i < PROCESS_NUM; i++) {
	  if(cfq_process_queue[i].req_num > 0) {
	  if(cfq_process_queue[i].head->trace_time < min) {
	  min = cfq_process_queue[i].head->trace_time;
	  min_index = i;
	  }
	  }
	  }*/
	...
	// CFQ
	if(curr_pid == -1) { // just for first time
		for(i = 0; i < PROCESS_NUM; i++) {
			if(cfq_process_queue[i].req_num > 0) {
				curr_pid = i;
				cfq_process_queue[i].starttime = simtime;
				cfq_process_queue[i].active = 1;
				break;
			}
		}
	}
	ASSERT(curr_pid != -1);
	cfq_process_queue[curr_pid].timeslice = TIMESLICE - (simtime - cfq_process_queue[curr_pid].starttime);
	if(cfq_process_queue[curr_pid].timeslice > 0 && cfq_process_queue[curr_pid].quota > 0) {
		if(cfq_process_queue[curr_pid].req_num > 0) {
			min_index = curr_pid;
		} else {
			min_index = -1;
		}
	} else { // round-robin, do another process
		cfq_process_queue[curr_pid].active = 0;

		min_index = -1;
		for(i = curr_pid + 1; i < PROCESS_NUM; i++) {
			if(cfq_process_queue[i].req_num > 0) {
				min_index = i;
				curr_pid = i;
				cfq_process_queue[i].starttime = simtime;
	...
	//	printf("curr_pid: %d\n", curr_pid);
	//// for original time calculating ////
	if ((queue->base.listlen - queue->base.numoutstanding) > 0) { // enter
		tmp = cfq_queue_remove_from_head(min_index); // add by wendy
		// update original subqueue info
	}
	...
	if(tmp) {
		totalreq_in_ioqueue--;
		ioqueue_update_subqueue_statistics(&queue->base);
		//		printf("simtime: %lf, pid: %d, trace_time: %lf, blkno: %d, bcount: %d\n", simtime, tmp->pid, tmp->trace_time, tmp->blkno, tmp->bcount);
		if (tmp->starttime_iobuf == -1.0) {
			tmp->starttime_iobuf = simtime;
		}
		// update original subqueue info
		stat_update(&queue->base.instqueuelen, (double)(queue->base.iobufcnt - queue->base.reqoutstanding));
		queue->base.reqoutstanding++;
		//		printf("cfq_queue_get_next_request: queue->debug_name: %d queue->base.reqoutstanding: %d blkno: %d pid: %d\n", queue->debug_name, queue->base.reqoutstanding, tmp->blkno, tmp->pid);
		queue->base.numoutstanding++;
		stat_update(&queue->base.qtimestats, (tmp->starttime_iobuf - tmp->time));
		tmp->time_iob_un = tmp->time;
		queue->base.num_scheduling_decisions++;

		// update original ioqueue info
		queue->lastsubqueue = 1; //IOQUEUE_BASE
		int numout = queue->base.numoutstanding + queue->timeout.numoutstanding + queue->priority.numoutstanding;
		//		printf("cfq_queue_get_next_request  numoutstanding: %d maxoutstanding: %d\n", numout, queue->maxoutstanding);
		if (numout > queue->maxoutstanding) {
			queue->maxoutstanding = numout;
			//		printf("change!! cfq_queue_get_next_request  numoutstanding: %d maxoutstanding: %d\n", numout, queue->maxoutstanding);
		}
		tmp->time = 0.0;
	}
	//// end original time calculating ////
...
void cfq_queue_remove_completed_request(struct ioq *queue, ioreq_event* curr)
{
	pending_request--;
	ASSERT(pending_request >= 0);

	all_time_calculate(curr->trace_time, simtime);
	ssd_time_calculate(curr->ssd_arrive_time, simtime, curr->flags);

	all_time_calculate_per_process(curr->pid, curr->flags, curr->trace_time, simtime);
	total_complete_req++;

	//	printf("cfq_queue_remove_completed_request\n");
	//// for original time calculating ////
	// update original ioqueue info
	queue->idlestart = simtime;
	...
	// update original subqueue info
	ioqueue_update_subqueue_statistics(&queue->base);
	curr->time = curr->starttime_iobuf;
	queue->base.iobufcnt--;
	queue->base.reqoutstanding--;
	//printf("cfq_queue_remove_completed_request: queue->debug_name: %d queue->base.reqoutstanding: %d blkno: %d pid: %d\n", queue->debug_name, queue->base.reqoutstanding, curr->blkno, curr->pid);
	//	printf("cfq_queue_remove_completed_request  blkno: %d pid: %d trace_time: %lf\n", curr->blkno, curr->pid, curr->trace_time);
	//	cfq_queue_print();
	queue->base.listlen--;
	if (curr->flags & READ) {
		queue->base.readlen--;
	...
	queue->base.numoutstanding--;
	queue->base.numcomplete++;

	stat_update(&queue->base.accstats, (simtime - curr->starttime_iobuf));
	disksim->lastphystime = simtime - curr->starttime_iobuf;
	stat_update(&queue->base.outtimestats, (simtime - curr->time_iob_un));

	if (curr->flags & READ) {
		if (curr->flags & TIME_CRITICAL) {
			stat_update(&queue->base.critreadstats, (simtime - curr->time_iob_un));
		} else {
			stat_update(&queue->base.nocritreadstats, (simtime - curr->time_iob_un));
		}
	} else {
		if (curr->flags & TIME_CRITICAL) {
			stat_update(&queue->base.critwritestats, (simtime - curr->time_iob_un));
		} else {
			stat_update(&queue->base.nocritwritestats, (simtime - curr->time_iob_un));
		}
	}
...
void fios_queue_print(void)
{
	int i;
	printf("read:  ");
	for(i = 0; i < PROCESS_NUM; i++) {
		printf("%d ", fios_process_read_queue[i].req_num);
	}
	printf("\n");

	printf("write: ");
	for(i = 0; i < PROCESS_NUM; i++) {
		printf("%d ", fios_process_write_queue[i].req_num);
	}
	printf("\n");

	printf("timeslice: ");
	for(i = 0; i < PROCESS_NUM; i++) {
		printf("%.2lf ", fios_process_queue_timeslice[i]);
	}
	printf("\n");
...
void fios_replenish(void)
{
//	printf("fios_replenish\n");
//	fios_queue_print();
	int i;
	ioreq_event *tmp;
	for(i = 0; i < PROCESS_NUM; i++) {
		fios_process_queue_timeslice[i] = TIMESLICE;
	}
}

int fios_able_to_dispatch(int pid, u_int flags)
{
	ioreq_event* tmp;

	ASSERT(pid >= 0 && pid < PROCESS_NUM);

	double tmp_timeslice;

	if(flags & READ) {
		ASSERT(fios_process_read_queue[pid].head != NULL);
		tmp = fios_process_read_queue[pid].head;
		ASSERT(tmp != NULL);

		tmp_timeslice = (double)tmp->bcount/pagesize_in_disksim*READ_TIMESLICE;
	} else { // write
		ASSERT(fios_process_write_queue[pid].head != NULL);
		tmp = fios_process_write_queue[pid].head;
		ASSERT(tmp != NULL);

		tmp_timeslice = (double)tmp->bcount/pagesize_in_disksim*WRITE_TIMESLICE;
	}
	ASSERT(tmp_timeslice > 0.0);

	if(tmp_timeslice > TIMESLICE) { // exceed timeslice limitted
		if(tmp->exceed_timeslice == 0) {
			tmp->exceed_timeslice = 1;
			tmp->dispatch_time = simtime;
		}
	}


	if((tmp->exceed_timeslice) && (simtime - tmp->dispatch_time > DELAY_DISPATCH) && fios_process_queue_timeslice[pid] > 0) {
		return 1;
	} else if(fios_process_queue_timeslice[pid] < tmp_timeslice) {
		return 0;
	} else {
		return 1;
	}
...
int fios_need_to_replenish(void)
{
	int i;
	int sum = 0;
	for(i = 0; i < PROCESS_NUM; i++) {
		sum += fios_process_read_queue[i].req_num + fios_process_write_queue[i].req_num;
	}
	...
	for(i = 0; i < PROCESS_NUM; i++) {
		if(fios_process_read_queue[i].req_num > 0) {
			if(fios_able_to_dispatch(i, READ)) {
				return 0;
			}
		}

		if(fios_process_write_queue[i].req_num > 0) {
			if(fios_able_to_dispatch(i, WRITE)) {
				return 0;
			}
		}
	}
...
void fios_queue_initialize(void)
{
	int i;
	for(i = 0; i < PROCESS_NUM; i++) {
		fios_process_read_queue[i].pid = i;
		fios_process_read_queue[i].req_num = 0;
		fios_process_read_queue[i].head = fios_process_read_queue[i].tail = NULL;

		fios_process_read_queue[i].active = 0;
		fios_process_read_queue[i].anticipation_time = 0;

		fios_process_write_queue[i].pid = i;
		fios_process_write_queue[i].req_num = 0;
		fios_process_write_queue[i].head = fios_process_write_queue[i].tail = NULL;

		fios_process_write_queue[i].active = 0;
		fios_process_write_queue[i].anticipation_time = 0;

		fios_process_queue_timeslice[i] = TIMESLICE;
	}
	find_light = 0;
	find_light_time = 0;
	in_anticipation_period = 0;
	in_read_anticipation_period = 0;
	...
	read_anti_counter = 0;
	read_anti_good = 0;
	read_anti_dispatch = 0;

}

void fios_queue_add_new_request(struct ioq *queue, ioreq_event *curr)
{
	totalreq_in_ioqueue++;
	if(!fios_process_flag) {
		fios_process_flag = 1;
		fios_process_time_start = simtime;
	}
//	printf("fios_queue_add_new_request  curr->trace_time: %lf, curr->blkno: %d, curr->pid: %d, curr->bcount: %d\n", curr->trace_time, curr->blkno, curr->pid, curr->bcount);
...
//	printf("fios_queue_add new pending: %d\n", pending_request);
	if(curr->flags & READ) { // add to fios_process_read_queue
		fios_process_read_queue[pid].pid = pid;
		fios_process_read_queue[pid].req_num++;

		// sort by time
		 if(fios_process_read_queue[pid].head == NULL && fios_process_read_queue[pid].tail == NULL) {
		   curr->prev = curr->next = NULL;
		   fios_process_read_queue[pid].head = curr;
		   } else {
		   curr->prev = curr->next = NULL;
		   fios_process_read_queue[pid].tail->next = curr;
		   curr->prev = fios_process_read_queue[pid].tail;
		   }

		   fios_process_read_queue[pid].tail = curr;

		// sort by address
		/*if(fios_process_read_queue[pid].head == NULL && fios_process_read_queue[pid].tail == NULL) {

			ASSERT(fios_process_read_queue[pid].head == NULL);
			ASSERT(fios_process_read_queue[pid].tail == NULL);

			curr->prev = curr->next = NULL;
			fios_process_read_queue[pid].head = curr;
			fios_process_read_queue[pid].tail = curr;

		} else if(curr->blkno < fios_process_read_queue[pid].head->blkno) {

			ASSERT(fios_process_read_queue[pid].head != NULL);
			ASSERT(fios_process_read_queue[pid].tail != NULL);

			fios_process_read_queue[pid].head->prev = curr;
			curr->prev = NULL;
			curr->next = fios_process_read_queue[pid].head;
			fios_process_read_queue[pid].head = curr;

		} else if(curr->blkno >= fios_process_read_queue[pid].tail->blkno) {
	...
	} else { // add to fios_porcess_write_queue
		fios_process_write_queue[pid].pid = pid;
		fios_process_write_queue[pid].req_num++;

		// sort by time
		 if(fios_process_write_queue[pid].head == NULL && fios_process_write_queue[pid].tail == NULL) {
		   curr->prev = curr->next = NULL;
		   fios_process_write_queue[pid].head = curr;
		   } else {
		   curr->prev = curr->next = NULL;
		   fios_process_write_queue[pid].tail->next = curr;
		   curr->prev = fios_process_write_queue[pid].tail;
		   }

		   fios_process_write_queue[pid].tail = curr;

		// sort by address
		/*if(fios_process_write_queue[pid].head == NULL && fios_process_write_queue[pid].tail == NULL) {

			ASSERT(fios_process_write_queue[pid].head == NULL);
			ASSERT(fios_process_write_queue[pid].tail == NULL);

			curr->prev = curr->next = NULL;
			fios_process_write_queue[pid].head = curr;
			fios_process_write_queue[pid].tail = curr;

		} else if(curr->blkno < fios_process_write_queue[pid].head->blkno) {

			ASSERT(fios_process_write_queue[pid].head != NULL);
			ASSERT(fios_process_write_queue[pid].tail != NULL);

			fios_process_write_queue[pid].head->prev = curr;
			curr->prev = NULL;
			curr->next = fios_process_write_queue[pid].head;
			fios_process_write_queue[pid].head = curr;

		} else if(curr->blkno >= fios_process_write_queue[pid].tail->blkno) {
...
ioreq_event* fios_queue_remove_from_head(int pid)
{
	ioreq_event *tmp = NULL;
	double tmp_timeslice;
	...
	if(fios_process_read_queue[pid].req_num > 0) { // remove from read queue

		tmp = fios_process_read_queue[pid].head;

		ASSERT(tmp != NULL);
		fios_process_read_queue[pid].head = fios_process_read_queue[pid].head->next;

		if(fios_process_read_queue[pid].head == NULL) {
			fios_process_read_queue[pid].head = fios_process_read_queue[pid].tail = NULL;
		} else {
			fios_process_read_queue[pid].head->prev = NULL;
		}

		fios_process_read_queue[pid].req_num--;
		tmp_timeslice = (double)(tmp->bcount)/pagesize_in_disksim*READ_TIMESLICE;
		ASSERT(tmp_timeslice > 0);
		fios_process_queue_timeslice[pid] -= tmp_timeslice; // TODO


	} else { // remove from write queue

		tmp = fios_process_write_queue[pid].head;

		ASSERT(tmp != NULL);
		fios_process_write_queue[pid].head = fios_process_write_queue[pid].head->next;

		if(fios_process_write_queue[pid].head == NULL) {
			fios_process_write_queue[pid].head = fios_process_write_queue[pid].tail = NULL;
		} else {
			fios_process_write_queue[pid].head->prev = NULL;
		}
...
//	ASSERT(fios_process_queue_timeslice[pid] >= 0.0);

//	 printf("fios_queue_remove_from_head  tmp->trace_time: %lf  tmp->blkno: %d  tmp->pid: %d  bcount: %d  timeslice: %lf\n", tmp->trace_time, tmp->blkno, tmp->pid, tmp->bcount, fios_process_queue_timeslice[tmp->pid]);
//		fios_queue_print();
...
int get_all_read_req_num(void)
{
	int sum = 0;
	int i;

	for(i = 0; i < PROCESS_NUM; i++) {
		sum += fios_process_read_queue[i].req_num;
	}

	return sum;
}

double calculate_anticipation_time(int pid)
{
	double anti_time = 0.0;
	double alpha = 0.0;

	if(req_num_per_process[pid] != 0) {
		anti_time = overall_service_time_per_process[pid] / req_num_per_process[pid];
	} else {
		anti_time = ANTICIPATION_TIME;
	}

	if(fios_process_time_total != 0) {
		alpha = acc_light_loading_time / fios_process_time_total;
	} else {
		alpha = 0.5;
	}

	anti_time = anti_time * alpha / (1 - alpha);

	return anti_time;
}


void generate_anticipation_event(int pid)
{
	ioreq_event *temp;
...
//	printf("generate_anticipate_event\n");
	temp->pid = FAKE_EVENT;
	//temp->time = simtime + calculate_anticipation_time(pid); // TODO
    temp->time = simtime + ANTICIPATION_TIME;
	temp->devno = 0;
	temp->blkno = 8000;
	temp->bcount = 64;
	temp->flags = 0;
...
int fios_queue_get_min(void)
{
	...
	for(i = 0; i < PROCESS_NUM; i++) {
		if(fios_process_read_queue[i].req_num > 0 && fios_process_read_queue[i].head->trace_time < min && fios_able_to_dispatch(i, READ)) {
			choose_pid = i;
			min = fios_process_read_queue[i].head->trace_time;
		}
	}

	if(choose_pid == -1) {
		for(i = 0; i < PROCESS_NUM; i++) {
			if(fios_process_write_queue[i].req_num > 0 && fios_process_write_queue[i].head->trace_time < min && fios_able_to_dispatch(i, WRITE)) {
				choose_pid = i;
				min = fios_process_write_queue[i].head->trace_time;
			}
		}
	}
...
int check_read_requests(){
	int i;
	for(i = 0; i<PROCESS_NUM; i++){
		if(fios_process_read_queue[i].req_num > 0 && fios_able_to_dispatch(i, READ) ){
			return 1;
		}
	}
	return 0;
...
/* FIOS Dispatch function*/
ioreq_event* fios_queue_get_next_request(struct ioq *queue, int enter_time)
{
//	printf("simtime: %lf in fios_queue_get_next_request, enter_time: %d\n", simtime, enter_time);
//	printf("%d\n", ssd_waiting_pool_count());
	ioreq_event *tmp = NULL;

	int i, j;
	int choose_pid;

	int req_num,read_num,wri_num,sum2;
	int sum;

	double anticipation_tmp;

	int is_find_curr_pid;
/*	printf("curr: %d\n", curr_pid);
	for(i = 0; i < PROCESS_NUM; i++) {
		printf("%d ", fios_process_read_queue[i].req_num + fios_process_write_queue[i].req_num);
	}
	printf("\n");
	for(i = 0; i < PROCESS_NUM; i++) {
		printf("%lf ", fios_process_queue_timeslice[i]);
	}
	printf("\n");*/
...
//	print_die_state();
	...
	if(find_light) {
		find_light = 0;
		acc_light_loading_time += (simtime - find_light_time);
        //printf("simtime= %lf, find_load_time = %lf, acc_light_loading_time= %lf\n",simtime,find_light_time,acc_light_loading_time);
	}
	...
	if(curr_pid == -1) { // just for first time
		for(i = 0; i < PROCESS_NUM; i++) {
			req_num = fios_process_read_queue[i].req_num + fios_process_write_queue[i].req_num;
			if(req_num > 0) {
				curr_pid = i;
				break;
			}
...
/*	if(curr_pid == -1) { // just for the first time
		curr_pid = fios_queue_get_min();
		fios_process_read_queue[curr_pid].active = 1;
		fios_process_write_queue[curr_pid].active = 1;

		fios_process_read_queue[curr_pid].anticipation_time = simtime;
		fios_process_write_queue[curr_pid].anticipation_time = simtime;
	}*/
...
/* Grant ADD Part*/
	...
	/* check if need to refresh or not */
	/* if need, means a epoch switch, then anticipation */
	/* if not, then choose a request */

    //printf("read_anticipation = %d, anticipation_period = %d",in_read_anticipation_period,in_anticipation_period);
	if(in_read_anticipation_period){
		/* in the read anticipation period, only consider read rqeuests */
        //anticipation_tmp = ANTICIPATION_TIME - (simtime - anti_read_time);
        //printf("ANTICIPATION_TIME = %lf, simtime = %lf, anti_read_time = %lf\n",ANTICIPATION_TIME,simtime, anti_read_time);
        //printf("anti = %lf\n",anticipation_tmp);
		if(ANTICIPATION_TIME - (simtime - anti_read_time) > 0){
			/* try to find a read request */
			if(check_read_requests()){
				i = curr_pid;
				do{
					if(fios_process_read_queue[i].req_num > 0 && fios_able_to_dispatch(i, READ) ){
			...
			/* read anticipate is over, also consider write requests */
			in_read_anticipation_period = 0;
			if(check_read_requests()){
				i = curr_pid;
				do{
				...
				i = curr_pid;
				do{
					if(fios_process_write_queue[i].req_num > 0 && fios_able_to_dispatch(i, WRITE) ){
						curr_pid = i;
						choose_pid = curr_pid;
						break;
					}
	...
	/* if the anticipation period,  */
	else if(in_anticipation_period){
        anticipation_tmp = ANTICIPATION_TIME - (simtime - anti_time);
        //printf("ANTICIPATION_TIME = %lf, simtime = %lf, anti_time = %lf\n",ANTICIPATION_TIME,simtime, anti_time);
        //printf("simtime = %lf\n",simtime);
        //printf("anti = %lf\n",anticipation_tmp);
		if(ANTICIPATION_TIME - (simtime - anti_time) > 0){
			if(check_read_requests()){
				i = curr_pid;
				do{
					if(fios_process_read_queue[i].req_num > 0 && fios_able_to_dispatch(i, READ) ){
			...
			/* anti time over */
			in_anticipation_period = 0;
			if(fios_need_to_replenish()){
				fios_replenish();
                generate_anticipation_event(curr_pid);
			}
		}
	}
	else{
	...
	for(i = 0; i < PROCESS_NUM; i++) {
         if( fios_process_read_queue[i].req_num > 0 && fios_able_to_dispatch(i, READ)){
            read_num += fios_process_read_queue[i].req_num ;
         }
         if(fios_process_write_queue[i].req_num > 0 && fios_able_to_dispatch(i, WRITE)){
            wri_num += fios_process_write_queue[i].req_num;
         }
		req_num = fios_process_read_queue[i].req_num + fios_process_write_queue[i].req_num;
		sum += req_num;
        sum2 = read_num + wri_num;
	}
    //printf("sum=%d\n",sum);
    //printf("sum2=%d\n",sum2);
    //printf("choose_pid= %d, sum = %d, find_light= %d\n",choose_pid,sum,find_light);
	if(choose_pid == -1 && sum > 0) {
        //printf("simtime = %lf",simtime);
		if(ssd_waiting_pool_count() < totalDie) {
			find_light = 1;
			find_light_time = simtime;
			find_light_times++;
		} else {
			ssd_die_monitor();
			for(i = 0; i < totalDie; i++) {
				if(judge_die_light_loading(i)) {
					find_light = 1;
					find_light_time = simtime;
					find_light_times++;
					break;
    ...
    //printf("choose_pid= %d , find_light_time = %lf, acc_light_loading_time= %lf, total_req= %d\n",choose_pid,find_light_time,acc_light_loading_time,disksim->totalreqs);
/*
	if(find_light && !in_anticipation_period && !in_read_anticipation_period) {
		for(i = 0; i < PROCESS_NUM; i++) {
			j = (curr_pid + i) % PROCESS_NUM;
			req_num = fios_process_read_queue[j].req_num + fios_process_write_queue[j].req_num;
			//		printf("process queue: %d, req num: %d\n", j, req_num);
			if(req_num > 0) {
				if(fios_process_read_queue[j].req_num > 0) {
					choose_pid = j;
					curr_pid = (curr_pid + 1) % PROCESS_NUM;
					break;
				} else if(fios_process_write_queue[j].req_num > 0) {
					choose_pid = j;
					curr_pid = (curr_pid + 1) % PROCESS_NUM;
					break;
				}
		...
		//		printf("%lf %d %d\n", simtime, sum, get_waiting_pool_req_num());
	}
*/	//	printf("choose_pid: %d\n", choose_pid);

	//// for original time calculating ////
	if ((queue->base.listlen - queue->base.numoutstanding) > 0) { // enter
		tmp = fios_queue_remove_from_head(choose_pid); // add by wendy
		// update original subqueue info
		ioqueue_update_subqueue_statistics(&queue->base);
	}
	// if get any task, generate anticipation event, then add to intq
    //printf("anticipation_period: %d, _need_to_replenish: %d\n",in_anticipation_period, fios_need_to_replenish());

	if(tmp){
		int i,req_num;
		int wri_num=0,read_num=0;
		for(i=0;i<PROCESS_NUM;i++){
			if( fios_process_read_queue[i].req_num > 0 && fios_able_to_dispatch(i, READ))
				read_num += fios_process_read_queue[i].req_num;
			if( fios_process_write_queue[i].req_num > 0 && fios_able_to_dispatch(i, WRITE))
		...
		req_num = wri_num + read_num;
		/* check if epoch switch or not*/
		if(fios_need_to_replenish()){
			anti_time = simtime;
			generate_anticipation_event(choose_pid);
			in_anticipation_period = 1;
		}
		else if((tmp->flags & READ) && read_num ==0 && wri_num > 0){
			generate_anticipation_event(choose_pid);
			anti_read_time = simtime;
			in_read_anticipation_period = 1;
		}
        else if(req_num > 0){
            //printf("simtime: %lf\n",simtime);
            generate_anticipation_event(choose_pid);
        }
		/*if(tmp->trace_time == 201516.313600 ){
			printf("dispatch here\n");
		}*/
		//printf("req_num: %d\n",req_num);
	}
	...
	if(tmp) {
		totalreq_in_ioqueue--;
//		printf("simtime: %lf, time: %lf, blkno: %d, pid: %d, timeslice: %lf, bcount: %d, flags: %d\n", simtime, tmp->trace_time, tmp->blkno, tmp->pid, fios_process_queue_timeslice[tmp->pid], tmp->bcount, tmp->flags);
	...
	//printf("totalreq_in_ioqueue: %d, NCQ: %d\n", totalreq_in_ioqueue, NCQ_command_size);
    /*if(choose_pid == 1)
        printf("curr_trace time = %lf, dispatch time = %lf\n",tmp->trace_time,simtime);*/
    return tmp;
}

void fios_queue_remove_completed_request(struct ioq *queue, ioreq_event* curr)
{
    int i;
    int req_num;
    int sum = 0;
	pending_request--;
	//printf("fios_queue_remove pending: %d\n", pending_request);
	...
	all_time_calculate_per_process(curr->pid, curr->flags, curr->trace_time, simtime);
	all_service_time_calculate_per_process(curr->pid, curr->ssd_arrive_time, simtime);

	total_complete_req++;

//	printf("fios_queue_remove_completed_request  curr->trace_time: %lf, curr->blkno: %d, curr->pid: %d\n", curr->trace_time, curr->blkno, curr->pid);
...
void wendy_queue_print(void)
{
	int i;
	printf("read:  ");
	for(i = 0; i < PROCESS_NUM; i++) {
		printf("%d ", wendy_process_read_queue[i].req_num);
	}
	printf("\n");

	printf("write: ");
	for(i = 0; i < PROCESS_NUM; i++) {
		printf("%d ", wendy_process_write_queue[i].req_num);
	}
	printf("\n");

	printf("timeslice: ");
	for(i = 0; i < PROCESS_NUM; i++) {
		printf("%.2lf ", wendy_process_queue_timeslice[i]);
	}
...
void wendy_replenish(void)
{
	int i;
	for(i = 0; i < PROCESS_NUM; i++) {
	//	if(wendy_process_queue_timeslice[i] < 0) {
	//		wendy_process_queue_timeslice[i] += TIMESLICE;
	//	} else {
			wendy_process_queue_timeslice[i] = TIMESLICE;
	//	}
	}
	replenish_time = simtime;
	// set_all_to_curr_period();
}

int wendy_able_to_dispatch(int pid, u_int flags)
{
	ioreq_event* tmp;
	...
	if(flags & READ) {
		ASSERT(wendy_process_read_queue[pid].head != NULL);
		tmp = wendy_process_read_queue[pid].head;
		ASSERT(tmp != NULL);

		tmp_timeslice = (double)tmp->bcount/pagesize_in_disksim*READ_TIMESLICE;
	} else { // write
		ASSERT(wendy_process_write_queue[pid].head != NULL);
		tmp = wendy_process_write_queue[pid].head;
		ASSERT(tmp != NULL);
	...
	if(wendy_process_queue_timeslice[pid] < tmp_timeslice) {
		return 0;
	} else {
		return 1;
	}
...
int wendy_need_to_replenish(void)
{
	int i;
	int sum = 0;
	for(i = 0; i < PROCESS_NUM; i++) {
		sum += wendy_process_read_queue[i].req_num + wendy_process_write_queue[i].req_num;
	}
	...
	for(i = 0; i < PROCESS_NUM; i++) {
		if(wendy_process_read_queue[i].req_num > 0) {
			if(wendy_able_to_dispatch(i, READ)) {
				return 0;
			}
		}

		if(wendy_process_write_queue[i].req_num > 0) {
			if(wendy_able_to_dispatch(i, WRITE)) {
				return 0;
			}
		}
	}
...
void wendy_queue_initialize(void)
{
	int i;
	for(i = 0; i < PROCESS_NUM; i++) {
		wendy_process_read_queue[i].pid = i;
		wendy_process_read_queue[i].req_num = 0;
		wendy_process_read_queue[i].head = wendy_process_read_queue[i].tail = NULL;

		wendy_process_write_queue[i].pid = i;
		wendy_process_write_queue[i].req_num = 0;
		wendy_process_write_queue[i].head = wendy_process_write_queue[i].tail = NULL;
...
void wendy_queue_add_new_request(struct ioq *queue, ioreq_event *curr)
{
	totalreq_in_ioqueue++;
	//// for original time calculating ////
	int listlen;
	...
	if(curr->flags & READ) { // add to wendy_process_read_queue
		wendy_process_read_queue[pid].pid = pid;
		wendy_process_read_queue[pid].req_num++;

		// sort by time
		/*if(wendy_process_read_queue[pid].head == NULL && wendy_process_read_queue[pid].tail == NULL) {
			curr->prev = curr->next = NULL;
			wendy_process_read_queue[pid].head = curr;
		} else {
			curr->prev = curr->next = NULL;
			wendy_process_read_queue[pid].tail->next = curr;
			curr->prev = wendy_process_read_queue[pid].tail;
		}

		wendy_process_read_queue[pid].tail = curr;*/

		// sort by address
		if(wendy_process_read_queue[pid].head == NULL && wendy_process_read_queue[pid].tail == NULL) {

			ASSERT(wendy_process_read_queue[pid].head == NULL);
			ASSERT(wendy_process_read_queue[pid].tail == NULL);

			curr->prev = curr->next = NULL;
			wendy_process_read_queue[pid].head = curr;
			wendy_process_read_queue[pid].tail = curr;

		} else if(curr->blkno < wendy_process_read_queue[pid].head->blkno) {

			ASSERT(wendy_process_read_queue[pid].head != NULL);
			ASSERT(wendy_process_read_queue[pid].tail != NULL);

			wendy_process_read_queue[pid].head->prev = curr;
			curr->prev = NULL;
			curr->next = wendy_process_read_queue[pid].head;
			wendy_process_read_queue[pid].head = curr;

		} else if(curr->blkno >= wendy_process_read_queue[pid].tail->blkno) {
	...
	} else { // add to wendy_porcess_write_queue
		wendy_process_write_queue[pid].pid = pid;
		wendy_process_write_queue[pid].req_num++;

		// sort by time
		/*if(wendy_process_write_queue[pid].head == NULL && wendy_process_write_queue[pid].tail == NULL) {
			curr->prev = curr->next = NULL;
			wendy_process_write_queue[pid].head = curr;
		} else {
			curr->prev = curr->next = NULL;
			wendy_process_write_queue[pid].tail->next = curr;
			curr->prev = wendy_process_write_queue[pid].tail;
		}

		wendy_process_write_queue[pid].tail = curr;*/

		// sort by address
		if(wendy_process_write_queue[pid].head == NULL && wendy_process_write_queue[pid].tail == NULL) {

			ASSERT(wendy_process_write_queue[pid].head == NULL);
			ASSERT(wendy_process_write_queue[pid].tail == NULL);

			curr->prev = curr->next = NULL;
			wendy_process_write_queue[pid].head = curr;
			wendy_process_write_queue[pid].tail = curr;

		} else if(curr->blkno < wendy_process_write_queue[pid].head->blkno) {

			ASSERT(wendy_process_write_queue[pid].head != NULL);
			ASSERT(wendy_process_write_queue[pid].tail != NULL);

			wendy_process_write_queue[pid].head->prev = curr;
			curr->prev = NULL;
			curr->next = wendy_process_write_queue[pid].head;
			wendy_process_write_queue[pid].head = curr;

		} else if(curr->blkno >= wendy_process_write_queue[pid].tail->blkno) {
...
ioreq_event* wendy_queue_remove_from_head(int pid, int find_light, int exceed_write_deadline, int exceed_write_starve)
{
	ioreq_event *tmp = NULL;
	double tmp_timeslice;
	...
	if(wendy_process_read_queue[pid].req_num > 0 && !exceed_write_deadline && !exceed_write_starve) { // remove from read queue

		tmp = wendy_process_read_queue[pid].head;

		ASSERT(tmp != NULL);
		if(wendy_able_to_dispatch(pid, READ)) {
			tmp->access_period = SSD_REQ_CURR;
		} else {
			tmp->access_period = SSD_REQ_NEXT;
		}

		wendy_process_read_queue[pid].head = wendy_process_read_queue[pid].head->next;

		if(wendy_process_read_queue[pid].head == NULL) {
			wendy_process_read_queue[pid].head = wendy_process_read_queue[pid].tail = NULL;
		} else {
			wendy_process_read_queue[pid].head->prev = NULL;
		}

		wendy_process_read_queue[pid].req_num--;
		tmp_timeslice = (double)(tmp->bcount)/pagesize_in_disksim*READ_TIMESLICE;
		ASSERT(tmp_timeslice > 0);
		wendy_process_queue_timeslice[pid] -= tmp_timeslice; // TODO
		...
		ASSERT(tmp != NULL);
		if(wendy_able_to_dispatch(pid, WRITE)) {
			tmp->access_period = SSD_REQ_CURR;
		} else {
			tmp->access_period = SSD_REQ_NEXT;
		}

		wendy_process_write_queue[pid].head = wendy_process_write_queue[pid].head->next;

		if(wendy_process_write_queue[pid].head == NULL) {
			wendy_process_write_queue[pid].head = wendy_process_write_queue[pid].tail = NULL;
		} else {
			wendy_process_write_queue[pid].head->prev = NULL;
		}
	 ...
	 */
	//	printf("wendy_queue_remove_from_head  tmp->trace_time: %lf  tmp->blkno: %d  tmp->pid: %d  bcount: %d  timeslice: %lf\n", tmp->trace_time, tmp->blkno, tmp->pid, tmp->bcount, wendy_process_queue_timeslice[tmp->pid]);
	//	wendy_queue_print();
...
int wendy_queue_get_min(void)
{
	...
	for(i = 0; i < PROCESS_NUM; i++) {
		if(wendy_process_read_queue[i].req_num > 0 && wendy_process_read_queue[i].head->trace_time < min/* && wendy_able_to_dispatch(i, READ)*/) {
			choose_pid = i;
			min = wendy_process_read_queue[i].head->trace_time;
		}
	}

	if(choose_pid == -1) {
		for(i = 0; i < PROCESS_NUM; i++) {
			if(wendy_process_write_queue[i].req_num > 0 && wendy_process_write_queue[i].head->trace_time < min /*&& wendy_able_to_dispatch(i, WRITE)*/) {
				choose_pid = i;
				min = wendy_process_write_queue[i].head->trace_time;
			}
		}
	}
...
int monitor_write_queue_deadline(void)
{
	int i;
	double max_time = -1;
	double tmp_time;
	...
	for(i = 0; i < PROCESS_NUM; i++) {
		if(wendy_process_write_queue[i].req_num > 0 && wendy_able_to_dispatch(i, WRITE)) {
			tmp_time = simtime - wendy_process_write_queue[i].head->trace_time;
			if(tmp_time > WRITE_DEADLINE) {
				wendy_process_write_queue[i].head->exceed_deadline = 1;
...
int monitor_write_queue_starve(void)
{
	int i;
	int max_starve = -1;
	int tmp_starve;
	...
	for(i = 0; i < PROCESS_NUM; i++) {
		if(wendy_process_write_queue[i].req_num > 0 && wendy_able_to_dispatch(i, WRITE)) {
			tmp_starve = wendy_process_write_queue[i].head->read_over_write_times;
			if(tmp_starve > WRITE_STARVE) {
				wendy_process_write_queue[i].head->exceed_starve = 1;
				if(tmp_starve > max_starve) {
					max_starve = tmp_starve;
					max_pid = i;
				}
			}
		}
...
int wendy_check_read_requests(){
	int i;
	for(i = 0; i<PROCESS_NUM; i++){
		if(wendy_process_read_queue[i].req_num > 0 && wendy_able_to_dispatch(i, READ) ){
			return 1;
		}
	}
	return 0;
...
ioreq_event* wendy_queue_get_next_request(struct ioq *queue, int enter_time)
{
//	printf("simtime: %lf in fios_queue_get_next_request, enter_time: %d\n", simtime, enter_time);
//	printf("%d\n", ssd_waiting_pool_count());
	ioreq_event *tmp = NULL;
	ioreq_event *write_tmp = NULL;
	...
	int exceed_write_deadline = 0;
	int exceed_write_starve = 0;
...
//	print_die_state();
	...
	if(curr_pid == -1) { // just for first time
		for(i = 0; i < PROCESS_NUM; i++) {
			req_num = wendy_process_read_queue[i].req_num + wendy_process_write_queue[i].req_num;
			if(req_num > 0) {
				curr_pid = i;
				break;
			}
...
/* Grant ADD Part*/
	...
	if(in_read_anticipation_period){
		...
		if(ANTICIPATION_TIME - (simtime - anti_read_time) > 0){
			...
			/* read anticipate is over, also consider write requests */
			in_read_anticipation_period = 0;
            if(IO_WRITE_DEADLINE_SCHEDULER == 1) {
                choose_pid = monitor_write_queue_deadline();
                if(choose_pid != -1) {
                    exceed_write_deadline = 1;
                }
            }

            if(IO_WRITE_STARVE == 1) {
                if(choose_pid == -1) {
                    choose_pid = monitor_write_queue_starve();
                    if(choose_pid != -1) {
                        exceed_write_starve = 1;
                    }
                }
            }
			if(choose_pid == -1) {
	...
	for(i = 0; i < PROCESS_NUM; i++) {
         if(wendy_process_read_queue[i].req_num > 0 && wendy_able_to_dispatch(i, READ)){
            read_num += wendy_process_read_queue[i].req_num ;
         }
         if(wendy_process_write_queue[i].req_num > 0 && wendy_able_to_dispatch(i, WRITE)){
            wri_num += wendy_process_write_queue[i].req_num;
         }
		req_num = wendy_process_read_queue[i].req_num + wendy_process_write_queue[i].req_num;
		sum += req_num;
        sum2 = read_num + wri_num;
	...
	if(find_light) {
		choose_pid = wendy_queue_get_min();
/*		for(i = 0; i < PROCESS_NUM; i++) {
			j = (curr_pid + i + 1) % PROCESS_NUM;
			req_num = wendy_process_read_queue[j].req_num + wendy_process_write_queue[j].req_num;
			//		printf("process queue: %d, req num: %d\n", j, req_num);
			if(req_num > 0) {
				if(wendy_process_read_queue[j].req_num > 0) {
					choose_pid = j;
					// curr_pid = j;
					break;
				} else if(wendy_process_write_queue[j].req_num > 0) {
					choose_pid = j;
					// curr_pid = j;
					break;
				}
    ...
    // calculate how many read over write occurs
	if(tmp && IO_WRITE_STARVE == 1) {
		if(tmp->flags & READ) {
			for(i = 0; i < PROCESS_NUM; i++) {
				if(wendy_process_write_queue[i].req_num > 0) {
					write_tmp = wendy_process_write_queue[i].head;
					while(write_tmp != NULL) {
						write_tmp->read_over_write_times++;
	...
	//// for original time calculating ////
	if ((queue->base.listlen - queue->base.numoutstanding) > 0) { // enter
		tmp = wendy_queue_remove_from_head(choose_pid, find_light, exceed_write_deadline, exceed_write_starve); // add by wendy
		// update original subqueue info
		ioqueue_update_subqueue_statistics(&queue->base);
	}
	// if get any task, generate anticipation event, then add to intq
...
void wendy_queue_remove_completed_request(struct ioq *queue, ioreq_event *curr)
{
	pending_request--;
	ASSERT(pending_request >= 0);
	...
	//printf("wendy_queue_remove_completed_request  curr->trace_time: %lf, curr->blkno: %d, curr->pid: %d\n", curr->trace_time, curr->blkno, curr->pid);
...
void oracle_queue_initialize(void)
{
	int i;
	for(i = 0; i < PROCESS_NUM; i++) {
		wendy_process_read_queue[i].pid = i;
...
void oracle_queue_add_new_request(struct ioq *queue, ioreq_event *curr)
{
	totalreq_in_ioqueue++;
	//// for original time calculating ////
	int listlen;
...
ioreq_event* oracle_queue_remove_from_head(int pid, int find_light)
{
	ioreq_event *tmp = NULL;
	double tmp_timeslice;
	...
	printf("\n");*/
	int lpn=0, ppn=0, c=0, d=0, pl=0, b=0, p=0;
	ioreq_event *iter = NULL;
	for(i = 0; i < PROCESS_NUM && tmp ==NULL; i++){
		if(wendy_process_read_queue[i].req_num > 0){
				iter = wendy_process_read_queue[i].head;
				ASSERT(wendy_process_read_queue[i].head != NULL);
				while(iter != NULL){
					lpn = iter->blkno /16;
					//printf("lpn: %d\n",lpn);
					mapping(lpn, &ppn, &c, &d, &pl, &b, &p);
					int die_id = ppn / 524288;
					//printf("dieid: %d\n",die_id);
					if(die_state[die_id] == 0){
						tmp = iter;	
						ASSERT(tmp != NULL);
						die_state[die_id] = 1;
						if(iter == wendy_process_read_queue[i].head){
							wendy_process_read_queue[i].head = wendy_process_read_queue[i].head->next;
							if(wendy_process_read_queue[i].head == NULL) {
								wendy_process_read_queue[i].head = wendy_process_read_queue[i].tail = NULL;
							} else {
								wendy_process_read_queue[i].head->prev = NULL;
							}
						}
						else if(iter->next == NULL){
							wendy_process_read_queue[i].tail = iter->prev;
							wendy_process_read_queue[i].tail->next = NULL;
							ASSERT(tmp !=NULL);
						}
						else{
							ioreq_event *t = NULL;
						...
						wendy_process_read_queue[i].req_num--;
						if(wendy_process_read_queue[i].req_num > 0){
							ASSERT(wendy_process_read_queue[i].head != NULL);
						}
						//printf("queue: %d, num: %d\n",i,wendy_process_read_queue[i].req_num);
						break;
					}
					else{
						iter = iter->next;
		...
		if(tmp == NULL && wendy_process_write_queue[i].req_num > 0){
				iter = wendy_process_write_queue[i].head;
				while(iter != NULL){
					lpn = iter->blkno /16;
					mapping(lpn, &ppn, &c, &d, &pl, &b, &p);
					int die_id = ppn / 524288;
					if(die_state[die_id] == 0){
						tmp = iter;	
						die_state[die_id] = 1;
						if(iter == wendy_process_write_queue[i].head){
							wendy_process_write_queue[i].head = wendy_process_write_queue[i].head->next;
							if(wendy_process_write_queue[i].head == NULL) {
								wendy_process_write_queue[i].head = wendy_process_write_queue[i].tail = NULL;
							} else {
								wendy_process_write_queue[i].head->prev = NULL;
							}
						}
						else if(iter->next == NULL){
							iter->prev->next = NULL;
							wendy_process_write_queue[i].tail = iter->prev;
							ASSERT(tmp != NULL);
						}
						else{
							ioreq_event *t;
						...
						wendy_process_write_queue[i].req_num--;
						if(wendy_process_write_queue[i].req_num >0){
							ASSERT(wendy_process_write_queue[i].head != NULL);
						}
						//printf("queue: %d, num: %d\n",i,wendy_process_write_queue[i].req_num);
						break;
					}
					else{
						iter = iter->next;
	...
	if(tmp == NULL){
	if(wendy_process_read_queue[pid].req_num > 0) { // remove from read queue
		//if(wendy_process_read_queue[pid].head == NULL)
			//printf("FUCK\n");
		tmp = wendy_process_read_queue[pid].head;
...
ioreq_event* oracle_queue_get_next_request(struct ioq *queue)
{
//	printf("wendy_queue_get_next_request\n");
//	printf("%d\n", ssd_waiting_pool_count());
	ioreq_event *tmp = NULL;
...
//		printf("process queue: %d, req num: %d\n", j, req_num);
	...
	//// for original time calculating ////
//	if ((queue->base.listlen - queue->base.numoutstanding) > 0) { // enter
		tmp = oracle_queue_remove_from_head(choose_pid, find_light); // add by wendy
		// update original subqueue info
		ioqueue_update_subqueue_statistics(&queue->base);
//	}
...
//		printf("wendy_queue_get_next_request null\n");
//		wendy_queue_print();
	}
	//// end original time calculating ////
...
void oracle_queue_remove_completed_request(struct ioq *queue, ioreq_event *curr)
{
	pending_request--;
	ASSERT(pending_request >= 0);

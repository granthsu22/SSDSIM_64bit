...
// add by wendy
static void trace_translate(ioreq_event *curr) {
	int new_blkno;
	int new_bcount;
	int page_size = pagesize_in_disksim;
	int max_lba = max_lba_in_disksim;

	ASSERT(page_size != 0); // make sure that the page size is reasonable
	ASSERT(max_lba != 0); // make sure that the max lba is reasonable

	curr->reasonable = 1;
	if(curr->time < 0 || curr->blkno < 0 || curr->bcount <= 0 || (curr->blkno + curr->bcount) > max_lba) {
		curr->reasonable = 0; // unreasonable
		return;
	}

	if(curr->blkno % page_size == 0) {
		// remain the same
		new_blkno = curr->blkno;
	} else {
		new_blkno = curr->blkno - curr->blkno % page_size;
	}

	if(curr->bcount % page_size == 0) {
		// remain the same
		new_bcount = curr->bcount;
	} else {
		new_bcount = (curr->bcount / page_size + 1) * page_size;
	}

	if((curr->blkno + curr->bcount) > (new_blkno + new_bcount)) {
		new_bcount = new_bcount + page_size;
	}
	
	if((new_blkno + new_bcount) >= max_lba_in_disksim) {
		curr->reasonable = 0;
		return;
	}

	curr->blkno = new_blkno;
	curr->bcount = new_bcount;
	curr->pid = curr->pid % PROCESS_NUM;
	
	if(CHOSEN_PID != -1 && curr->pid != CHOSEN_PID) {
		curr->reasonable = 0;
		return;
	}
	
	if(CHOSEN_TYPE != -1 && curr->flags != CHOSEN_TYPE) {
		curr->reasonable = 0;
		return;
	}
	...
	ASSERT(curr->time >= 0);
	ASSERT(curr->blkno >= 0);
	ASSERT(curr->bcount > 0);
	ASSERT((curr->blkno + curr->bcount) < max_lba_in_disksim);
...
//		printf("%s\n", line);
		if(USE_PID_TRACE) {
			if (sscanf(line, "%d %lf %d %d %d %x\n", &new->pid, &new->time, &new->devno, &new->blkno, &new->bcount, &new->flags) != 6) {
				fprintf(stderr, "Wrong number of arguments for I/O trace event type\n");
				fprintf(stderr, "line: %s", line);
				ddbg_assert(0);
			}
		...
		trace_translate(new); // wendy
	} while(new->reasonable == 0); // wendy
	...
	new->trace_time = new->time;


	ASSERT(new->time >= 0);
	ASSERT(new->bcount > 0);
	ASSERT(new->blkno >= 0);
	ASSERT((new->blkno + new->bcount) < max_lba_in_disksim);
...
//	printf("disksim_iotrace=> time: %lf, blkno: %d, bcount: %d\n", new->time, new->blkno, new->bcount);

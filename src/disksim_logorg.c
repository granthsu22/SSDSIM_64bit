...
static int logorg_overlap (logorg *currlogorg, int devno, ioreq_event *curr, long long int blksperpart)
{
	int calc;
...
{
//	printf("simtime: %lf, logorg_maprequest_update_stats: %d, totalreqs: %d\n", simtime, currlogorg->stat.outstanding, disksim->totalreqs);
	int i;
	ioreq_event *temp;
	int gen;
	double tpass;
...
{
//	printf("simtime: %lf, logorg_mapcomplete_update_stats: %d, totalreqs: %d\n", simtime, currlogorg->stat.outstanding, disksim->totalreqs);
...
//			printf("curr->devno: %d, logorgs[i]->arraydisk: %d\n", curr->devno, logorgs[i]->arraydisk); // wendy
//			printf("curr->blkno: %d, curr->bcount: %d, logorgs[i]->blksperpart: %lld, logorgs[i]->numdisks: %d\n", curr->blkno, curr->bcount, logorgs[i]->blksperpart, logorgs[i]->numdisks);
			...
			for(j = 0; j < logorgs[i]->actualnumdisks; j++) { // for (j = 0; j < logorgs[i]->numdisks; j++) 

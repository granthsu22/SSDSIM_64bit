...
void ssd_initialize (void)
{
	...
	for (i=0; i<numssds; i++) {
		...
		ssd_alloc_queues(currdisk); // wendy: why?!!
		...
		/*        if (!currdisk->inited) { */
		pagesize_in_disksim = currdisk->params.page_size; // wendy: for disksim_iotrace to know page size to align trace

		currdisk->numblocks = (long long int) currdisk->params.nelements *
		...
		currdisk->queue->debug_name = SSD_QUEUE;

		cfq_queue_initialize(); // add by wendy
		fios_queue_initialize(); // add by wendy
		wendy_queue_initialize(); // add by wendy
		ssd_statinit(i, TRUE);
		...
		/* modified by gengyouchen */ // can change confuguration
		ssd_fsm_init(currdisk);
		ssd_ftl_init(currdisk);
		ssd_scheduler_init(currdisk);
	}
}
